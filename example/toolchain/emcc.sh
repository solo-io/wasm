...
find . -name "*.d" -exec sed -e '2d' -e 's%\S*/external/emscripten_toolchain/upstream/emscripten/system/%external/emscripten_toolchain/upstream/emscripten/system/%' -i {} \;

# yet another hack till i can figure out how to make no-canonical-prefixes work
find . -name "*.d" -exec sed -e 's%\S*/external/envoy_wasm-tmp/api/wasm/cpp%external/envoy_wasm%' -i {} \;
